...
db_file = st.file_uploader(“📂 1. 上传【总账本.xlsx】”, type=[‘xlsx’], help=“包含客户余额、加工费规则等基础数据的总账本”)
daily_file = st.file_uploader(“📝 2. 上传【今日过磅单】”, type=[‘xls’, ‘xlsx’, ‘csv’], help=“当日的过磅明细单”)
...
if new_d.strip():
st.session_state[f”custom_drv_{t}”] = new_d.strip()
st.rerun()
...
```
    ...
    all_known_custs = sorted(list(set(df_bal['客户名称'].dropna().astype(str)) | set(df_hist['收货单位'].dropna().astype(str)) if not df_hist.empty else set(df_bal['客户名称'].dropna().astype(str))))
        ...
        if delivery_mask.any():
            ...
            with col_b2:
                with st.popover("⚙️ 批量设置运价", use_container_width=True):
                    ...
                    b_prc = st.number_input("统一设置运价", min_value=0.0, format="%.2f")
                    if st.button("应用到勾选的车辆", use_container_width=True):
            ...
            def render_truck_row(t):
                ...
                card_style = "done" if is_done else ""

                # 卡片容器
                st.markdown(f'<div class="truck-card {card_style}">', unsafe_allow_html=True)
                ...
                with c_info:
                    ...
                    with st.popover(f"{prefix}🚛 {t} · {count}车", use_container_width=True):
                ...
                d_sel = st.selectbox(f"司机 ({t})", opts, index=idx, key=f"d_sel_{t}", label_visibility="visible")
                if d_sel == "➕ 手动输入新司机...":
                    add_driver_modal(t)
                else:
    ...
    if not has_delivery:
        with st.expander("📦 额外运费补充（无配送可忽略）", expanded=False):
            f_val = st.number_input("额外运费(元):", value=0.0, format="%.2f", label_visibility="collapsed")
    ...
    with tab_income:
        ...
        for i in range(st.session_state.income_rows):
            st.markdown(f'<div class="finance-card">', unsafe_allow_html=True)
            if st.session_state.income_rows > 1:
                st.caption(f"收入记录 #{i+1}")

            # 客户名称（全宽）
            c_sel = st.selectbox("客户名称", cust_options, key=f"inc_c_sel_{i}")
            if c_sel == "➕ 手动输入新客户...":
                c_name = st.text_input("新客户名称", key=f"inc_c_new_{i}", placeholder="请输入客户名称")
            else:
            ...
            with ci1:
                i_type = st.selectbox("收入类型", ["微信", "现金", "银行卡", "其他"], key=f"inc_t_{i}")
            with ci2:
                i_amt = st.number_input("金额(元)", min_value=0.0, format="%.2f", key=f"inc_a_{i}")

            # 备注（全宽）
            i_note = st.text_input("备注（选填）", key=f"inc_n_{i}", placeholder="选填")
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    with tab_deposit:
        ...
        for i in range(st.session_state.deposit_rows):
            st.markdown(f'<div class="finance-card">', unsafe_allow_html=True)
            if st.session_state.deposit_rows > 1:
                st.caption(f"预存记录 #{i+1}")

            # 客户名称（全宽）
            c_sel = st.selectbox("客户名称 ", cust_options, key=f"dep_c_sel_{i}")
            if c_sel == "➕ 手动输入新客户...":
                c_name = st.text_input("新客户名称", key=f"dep_c_new_{i}", placeholder="请输入客户名称")
            else:
                c_name = c_sel if c_sel != "(不录入)" else ""
            ...
            with di1:
                i_type = st.selectbox("预存类型", ["预存微信", "银行卡", "预存现金", "其他"], key=f"dep_t_{i}")
            with di2:
                i_amt = st.number_input("金额(元) ", min_value=0.0, format="%.2f", key=f"dep_a_{i}")

            # 备注（全宽）
            i_note = st.text_input("备注（选填） ", key=f"dep_n_{i}", placeholder="选填")
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    for r in today_income_records:
        if r['客户名称'] != '零售客户':
            ...
            if c_name:
                collection_parts.append(f"{c_name}{i_type}:{amt_str}元")
            else:
                collection_parts.append(f"{i_type}:{amt_str}元")
