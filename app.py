    ...
    try:
        ...
        if '备注' in df_sales.columns:
            ...
            if delivery_mask.any():
                ...
                # 顶部操作行
                col_b1, col_b2, _ = st.columns([2.5, 3, 4.5])
                with col_b1:
                ...
                def render_truck_row(t):
                    ...
                    # 精确的 4列比例控制 (勾选框:5%, 车号:25%, 司机:45%, 运价:25%)
                    c_chk, c_info, c_drv, c_prc = st.columns([1, 4, 6.5, 4.5])
                    
                    with c_chk:
                        st.checkbox("", key=f"chk_{t}", label_visibility="collapsed")
                    
                    with c_info:
                        with st.popover(f"{icon} {t} · {count}车"):
                            st.dataframe(this_df[['单号', '重车时间', '收货单位', '货物名称', '净重']], hide_index=True)
        ...
        with tab_income:
            ...
            for i in range(st.session_state.income_rows):
                ...
                with col_a: i_amt = st.number_input(f"金额(元)", min_value=0.0, format="%.2f", key=f"inc_a_{i}", label_visibility=label_vis)
                with col_n:
        ...
        with tab_deposit:
            ...
            for i in range(st.session_state.deposit_rows):
                ...
                with col_a: i_amt = st.number_input(f"金额(元) ", min_value=0.0, format="%.2f", key=f"dep_a_{i}", label_visibility=label_vis)
                with col_n:
