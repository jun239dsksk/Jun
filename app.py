...
def create_template():
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        pd.DataFrame(columns=['单号', '重车时间', '车号', '货物名称', '净重', '单价', '金额', '收货单位', '过磅类型', '备注', '加工费单价', '加工费', '备注2']).to_excel(writer, sheet_name='过磅明细', index=False)
...
c1, c2 = st.columns(2)
with c1: db_file = st.file_uploader("📂 1. 上传【总账本.xlsx】", type=['xlsx'], help="包含客户余额、加工费规则等")
    ...
    try:
        ...
        if mixed_mask.any():
            ...
            for idx, row in df_daily_raw.iterrows():
                ...
                if mat in split_ratios:
                    ...
                    row['_pre_rounded'] = True 
                    new_rows.append(row)
            df_daily_raw = pd.DataFrame(new_rows)
            st.divider()

        # ⭐️ 核心突破：字典自动学习（收集未知的物料+单价组合）
        missing_rules = []
        valid_sales = df_daily_raw[df_daily_raw['过磅类型'].astype(str).str.strip() != '']
        unique_combinations = valid_sales[['货物名称', '单价']].drop_duplicates()
        ...
        for _, r in unique_combinations.iterrows():
            ...
            if not df_rules.empty:
                match = df_rules[(df_rules['物料名称'] == mat) & (df_rules['销售单价'] == price)]
                if not match.empty: is_exist = True
            
            if not is_exist:
                # 发现盲区，加入小本本，加工费单价设为 None（写入Excel就是留空）
                missing_rules.append({'物料名称': mat, '销售单价': price, '加工费单价': None})
                
        if missing_rules:
            new_rules_df = pd.DataFrame(missing_rules)
            df_rules = safe_concat([df_rules, new_rules_df])
            added_mats = ", ".join([f"{r['物料名称']}({r['销售单价']}元)" for r in missing_rules])
            st.info(f"🤖 **系统自动学习**：发现未记录的配方 `{added_mats}`。已自动添加到【加工费规则】底表！本次暂记为0，请在下载的新账本中补全。")
        ...
        with st.expander("💰 财务资金登记 (点击展开录入收入与预存)", expanded=False):
            ...
            with tab_deposit:
                ...
                for i in range(st.session_state.deposit_rows):
                    ...
                    with c_n:
                        i_note = text_input(f"备注 ", key=f"dep_n_{i}", label_visibility=label_vis, placeholder="选填")
