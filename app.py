...
def show_logs_dialog():
    ...
    m2.metric('        WARN', warn_c)
    m3.metric('        ERROR', err_c)
    st.markdown('---')
    filter_level = st.radio('筛选级别', ['全部', 'INFO', 'WARN', 'ERROR'], horizontal=True)
    filtered = logs if filter_level == '全部' else [l for l in logs if '[' + filter_level + ']
    if not filtered:
        st.info('暂⽆ ' + filter_level + ' 级别⽇志')
    else:
        lines_html = []
...
                  '<div style="font-family:monospace;font-size:12px;padding:2px 0;'
    ...
    st.download_button(
          '   导出⽇志 (.txt)',
          data=log_text,
          file_name='地磅⽇志_' + datetime.now().strftime('%Y%m%d_%H%M%S') + '.txt',
...
with st.sidebar:
    st.markdown('###       报表时间')
    report_date = st.date_input('报表⽇期', value=datetime.now(), label_visibility='collapsed')
    report_time = st.text_input('时间段', value='07:00-18:00', label_visibility='collapsed')
    st.divider()
    st.markdown('###       取整设置')
    st.caption('勾选 = 取整，否则保留2位⼩数')
    round_retail     = st.checkbox('零售(微信/现⾦)', value=True)
    round_sign       = st.checkbox('签单客户',        value=False)
    round_fee        = st.checkbox('加⼯费',         value=False)
    round_freight = st.checkbox('运费',              value=False)
    st.divider()
    if st.button('      查看 / 导出系统⽇志', use_container_width=True):
           show_logs_dialog()
    st.divider()
    st.markdown('###       空⽩模板')
...
def do_round(val, category=''):
    if pd.isna(val):
           return 0.0
    val = float(val)
    flags = {'retail': round_retail, 'sign': round_sign, 'fee': round_fee, 'freight': round_f
...
def parse_excel_date(val):
    ...
    try:
           f_val = float(val)
           if f_val > 30000:
                  return pd.to_datetime(f_val, unit='D', origin='1899-12-30').strftime('%Y-%m-%d %H
    except Exception:
           pass
    return str(val)
...
def create_template():
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
           pd.DataFrame(columns=['客户名称', '余额']).to_excel(writer, sheet_name='客户余额', index=
           pd.DataFrame(columns=['物料名称', '销售单价', '加⼯费单价']).to_excel(writer, sheet_name='
           pd.DataFrame(columns=['原始名称', '标准名称']).to_excel(writer, sheet_name='物料归类映射',
           pd.DataFrame(columns=['单号','重⻋时间','⻋号','货物名称','标准归类','净重','单价','⾦额',
                                     '收货单位','过磅类型','备注','加⼯费单价','加⼯费','备注2']
                          ).to_excel(writer, sheet_name='过磅明细', index=False)
           pd.DataFrame(columns=['单号','重⻋时间','⻋号','收货单位','货物名称','净重','司机姓名','运费单
                          ).to_excel(writer, sheet_name='公司配送-运费', index=False)
           pd.DataFrame(columns=['⽇期','客户名称','收⼊类型','⾦额','备注']
                          ).to_excel(writer, sheet_name='财务收⼊明细', index=False)
...
st.divider()
st.markdown('#####         第⼀步：上传业务⽂件')
c1, c2 = st.columns(2)
with c1:
    db_file          = st.file_uploader('总账本 (.xlsx)', type=['xlsx'])
with c2:
    daily_file = st.file_uploader('今⽇过磅单 (.xls/.xlsx/.csv)', type=['xls','xlsx','csv'])
...
try:
       ...
       if load_key not in st.session_state:
            ...
            add_log('INFO', '读取今⽇过磅单: ' + daily_file.name)
       ...
       def read_sheet(name, cols):
            if name in sheets:
                  ...
                  if missing:
                      add_log('WARN', '⼯作表 [' + name + '] 缺少列: ' + str(missing) + '，已⽤空表
                      return pd.DataFrame(columns=cols)
                  return df
            add_log('WARN', '总账本缺少⼯作表: [' + name + ']，使⽤空表')
            return pd.DataFrame(columns=cols)


       df_bal        = read_sheet('客户余额',        ['客户名称', '余额'])
       df_rules      = read_sheet('加⼯费规则',       ['物料名称', '销售单价', '加⼯费单价'])
       df_mapping = read_sheet('物料归类映射',        ['原始名称', '标准名称'])
       df_hist       = read_sheet('过磅明细',        ['单号'])
       df_freight = read_sheet('公司配送-运费', ['单号','重⻋时间','⻋号','收货单位','货物名称','净重
       df_income     = read_sheet('财务收⼊明细',     ['⽇期','客户名称','收⼊类型','⾦额','备注'])
       ...
       if daily_file.name.endswith('.csv'):
            ...
            add_log('INFO', '今⽇过磅单格式: CSV')
       else:
            df_daily_raw = pd.read_excel(daily_file)
            add_log('INFO', '今⽇过磅单格式: Excel')


       raw_count = len(df_daily_raw)
       if '状态' in df_daily_raw.columns:
            df_daily_raw = df_daily_raw[~df_daily_raw['状态'].astype(str).str.contains('作废|⽣
            filtered_count = raw_count - len(df_daily_raw)
            if filtered_count:
                  add_log('INFO', '过滤作废/⽣产记录 ' + str(filtered_count) + ' 条，剩余 ' + str(l
df_daily_raw['单号']           = df_daily_raw.get('单号', '').astype(str).str.replace('.0',
df_daily_raw['重⻋时间']         = df_daily_raw.get('重⻋时间', '').apply(parse_excel_date)
df_daily_raw['货物名称']         = df_daily_raw.get('货物名称', '未知物料').astype(str).str.st
df_daily_raw['净重']            = pd.to_numeric(df_daily_raw.get('净重', 0), errors='coerce
df_daily_raw['单价']            = pd.to_numeric(df_daily_raw.get('单价', 0), errors='coerce
df_daily_raw['⾦额']            = pd.to_numeric(df_daily_raw.get('⾦额', 0), errors='coerce
...
if mixed_mask.any():
    ...
    add_log('WARN', '检测到混合料 ' + str(len(mixed_mats)) + ' 种: ' + str(list(mixed_ma
    st.warning('      **第⼀关：检测到 ' + str(len(mixed_mats)) + ' 种混合料，请配置拆分⽐例**
    ...
    for mat in mixed_mats:
        ...
        with col_s1:
            pct1 = st.number_input('【' + mat + '】[' + p1 + '] 占⽐(%)',
                   min_value=1.0, max_value=99.0, value=50.0, step=1.0, key='s1_' + mat)
        with col_s2:
            st.number_input('[' + p2 + '] 占⽐(%)', value=100.0 - pct1, disabled=True,
        split_ratios[mat] = pct1


    split_confirmed = st.checkbox('      ⽐例已确认，⽴刻拆分并进⼊下⼀步', key='chk_split')
...
        mat1 = parts[0].strip()
        ...
        w_orig, p_orig, a_orig = row['净重'], row['单价'], row['⾦额']
        t_type, dh = str(row['过磅类型']), str(row['单号'])
        cat = 'retail' if ('微信' in t_type or '现⾦' in t_type) else ('sign' if '签单'
        if p_orig == 0 and a_orig == 0:
            w1 = round(w_orig * (pct1 / 100.0), 3)
            w2 = w_orig - w1
            r1['净重'], r1['单价'], r1['⾦额'] = w1, 0.0, 0.0
            r2['净重'], r2['单价'], r2['⾦额'] = w2, 0.0, 0.0
            ...
            r1['净重'], r1['单价'], r1['⾦额'] = w1, p_orig, a1
            r2['净重'], r2['单价'], r2['⾦额'] = w2, p_orig, a2
            r1['_pre_rounded'] = r2['_pre_rounded'] = True
        r1['货物名称'], r2['货物名称'] = mat1, mat2
        r1['汇报专⽤名称'] = r2['汇报专⽤名称'] = mat
        r1['备注2'] = r2['备注2'] = mat1 + ' ' + str(pct1) + '%+' + mat2 + ' ' + str(pc
...
# 第⼆关：物料归类映射
# ============================================================
df_daily_raw['标准归类']          = df_daily_raw['货物名称'].apply(lambda x: mapping_dict.get
df_daily_raw['汇报专⽤名称'] = df_daily_raw.apply(
    lambda r: r['标准归类'] if r['汇报专⽤名称'] == '' else r['汇报专⽤名称'], axis=1)
...
if unknown_mats:
    add_log('WARN', '发现未知物料 ' + str(len(unknown_mats)) + ' 种: ' + str(unknown_mat
    st.error('         **第⼆关：发现 ' + str(len(unknown_mats)) + ' 种未知物料，请指定⽗类**')
    map_options = ['(独⽴物料)', '(⾮销售/不计价)'] + sorted(known_standards)
    cols = st.columns(3)
    mapping_inputs = {}
    ...
    if not map_confirmed:
           add_log('WARN', '流程挂起：等待⽤户确认未知物料归类')
           st.stop()
    new_mapping_records = []
    for u_mat, sel_val in mapping_inputs.items():
           if sel_val != '(请选择...)':
               std_name = u_mat if sel_val == '(独⽴物料)' else sel_val
    ...
    if new_mapping_records:
           ...
           df_daily_raw['标准归类'] = df_daily_raw['货物名称'].apply(lambda x: mapping_dict
           df_daily_raw['汇报专⽤名称'] = df_daily_raw.apply(
               lambda r: r['标准归类'] if r['汇报专⽤名称'] == r['货物名称'] else r['汇报专⽤名
    add_log('INFO', '物料归类映射完成，新增 ' + str(len(new_mapping_records)) + ' 条映射记
...
def fix_shdw(row):
    ...
    if shdw in ('', 'nan'):
           if gb_type == '':
               return '内部单'
           if '微信' in gb_type or '现⾦' in gb_type:
               return '零售客户'
           return '未知客户'
    return shdw
...
missing_mask = (
    ...
    & (df_daily_raw['标准归类'] != '(⾮销售/不计价)')
)
if missing_mask.any():
    missing_groups = df_daily_raw[missing_mask].groupby(['收货单位', '货物名称'])
    add_log('WARN', '检测到 ' + str(missing_mask.sum()) + ' 条记录缺失单价，涉及 ' + str(l
    st.warning('      **第三关：' + str(missing_mask.sum()) + ' 条有效销售记录缺失单价，请补充
    cols = st.columns(4)
    price_inputs = {}
    for i, ((cust, mat), _) in enumerate(missing_groups):
          with cols[i % 4]:
              price_inputs[(cust, mat)] = st.number_input(
                   '**' + cust + '**\n' + mat + ' (元/吨)',
                   min_value=0.0, step=1.0, format='%.2f', key='miss_p_' + cust + '_' +
    price_confirmed = st.checkbox('      单价已全部补⻬', key='chk_price')
    st.divider()
    if not price_confirmed:
          add_log('WARN', '流程挂起：等待⽤户补充缺失单价')
          st.stop()
...
        missing_rules.append((mat, price))
    ...
    if match.empty or pd.isna(match.iloc[0]['加⼯费单价']) or str(match.iloc[0]['加⼯费单
        missing_rules.append((mat, price))


if missing_rules:
    add_log('WARN', '发现 ' + str(len(missing_rules)) + ' 条未收录加⼯费规则: ' + str(mis
    st.error('      **第四关：发现 ' + str(len(missing_rules)) + ' 条未收录加⼯费规则，请补⻬*
    cols = st.columns(4)
    fee_inputs = {}
    for i, (mat, price) in enumerate(missing_rules):
        with cols[i % 4]:
            fee_inputs[(mat, price)] = st.number_input(
                   '**' + mat + '**\n单价:' + str(price) + '元 → 加⼯费',
                   min_value=0.0, step=1.0, format='%.2f', key='miss_f_' + mat + '_' + s
    fee_confirmed = st.checkbox('      加⼯费已补⻬，完成最终核算', key='chk_fee')
    st.divider()
    if not fee_confirmed:
        add_log('WARN', '流程挂起：等待⽤户补充加⼯费规则')
        st.stop()
    new_rules = [{'物料名称': mat, '销售单价': price, '加⼯费单价': fee}
                    for (mat, price), fee in fee_inputs.items()]
    df_rules = safe_concat([df_rules, pd.DataFrame(new_rules)])
    for (mat, price), fee in fee_inputs.items():
        add_log('INFO', '新增加⼯费规则: [' + mat + '] 单价:' + str(price) + ' → 加⼯费:'
...
for _, row in df_daily_raw.iterrows():
    if row['标准归类'] == '(⾮销售/不计价)':
        ...
        new_amts.append(row['⾦额'])
    else:
        w, p, orig_a, t_type = row['净重'], row['单价'], row['⾦额'], str(row['过磅类型']
        exact = (w * p) if p > 0 else orig_a
        cat = 'retail' if ('微信' in t_type or '现⾦' in t_type) else ('sign' if '签单'
        new_amts.append(do_round(exact, cat))
df_daily_raw['⾦额'] = new_amts
def calc_fee_price(row):
     if str(row.get('过磅类型','')).strip() == '' or row.get('标准归类','') == '(⾮销售/不
           return 0.0
     match = df_rules[
           (df_rules['物料名称'].astype(str).str.strip() == str(row['标准归类']).strip())
           & (df_rules['销售单价'].astype(float) == float(row['单价']))
     ]
     return float(match.iloc[0]['加⼯费单价']) if not match.empty else 0.0


df_daily_raw['加⼯费单价'] = df_daily_raw.apply(calc_fee_price, axis=1)
df_daily_raw['加⼯费']            = df_daily_raw.apply(
     lambda r: do_round(r['净重'] * r['加⼯费单价'], 'fee') if r['加⼯费单价'] > 0 else 0.
add_log('INFO', '核算完成：总销售额 ' + str(round(df_daily_raw['⾦额'].sum(),2)) + '，总加


df_report_base = df_daily_raw.groupby('_orig_idx').agg({
     '单号': 'first', '重⻋时间': 'first', '⻋号': 'first',
     '收货单位': 'first', '汇报专⽤名称': 'first', '标准归类': 'first',
     '过磅类型': 'first', '备注': 'first',
     '净重': 'sum', '⾦额': 'sum',
...
driver_options = ['(未选择)'] + all_known_drivers + ['        ⼿动输⼊新司机...']


hist_custs = df_hist['收货单位'].dropna().astype(str).tolist() if '收货单位' in df_hist.
all_known_custs = sorted(set(df_bal['客户名称'].dropna().astype(str).tolist() + hist_cu
all_known_custs = [c for c in all_known_custs if c.strip() and c not in ('nan','内部单
cust_options = ['(不录⼊)'] + all_known_custs + ['        ⼿动输⼊新客户...']
...
expander_title = ('            检测到 ' + str(delivery_count) + ' ⻋公司配送，点击展开分配'
                        if has_delivery else '   公司配送与额外运费')
with st.expander(expander_title, expanded=False):
    if has_delivery:
        add_log('INFO', '检测到 ' + str(delivery_count) + ' ⻋公司配送记录')
        delivery_df      = df_sales_report[delivery_mask].copy()
        truck_counts     = delivery_df['⻋号'].value_counts()
        ...
        if not df_freight.empty:
            for _, r in df_freight.iterrows():
                if pd.notna(r.get('⻋号')) and pd.notna(r.get('司机姓名')):
                       mem_driver[str(r['⻋号'])] = str(r['司机姓名'])
        ...
        with col_b1:
            if st.button('      全选/反选', use_container_width=True):
                ...
                add_log('INFO', '批量' + ('全选' if not curr else '反选') + '配送⻋辆')
                st.rerun()
        with col_b2:
            with st.popover('       批量设置运价/司机', use_container_width=True):
                st.markdown('**批量分配司机**')
                b_drv = st.selectbox('统⼀分配司机', driver_options[:-1])
                if st.button('应⽤司机(仅勾选)', use_container_width=True):
                       for t in unique_trucks:
                           if st.session_state.get('chk_' + str(t), False) and b_drv !=
                               st.session_state['custom_drv_' + str(t)] = b_drv
                       add_log('INFO', '批量分配司机: ' + b_drv)
                       st.rerun()
                st.divider()
                st.markdown('**按客户设置运价**')
                b_cust = st.selectbox('⽬标', ['(对所有已勾选)'] + list(unique_delivery_
                b_prc     = st.number_input('运价', min_value=0.0, step=1.0, format='%.2f
                if st.button('应⽤运价', use_container_width=True):
                       for t in unique_trucks:
                           t_cust = delivery_df[delivery_df['⻋号'] == t].iloc[0]['收货单位
                           if b_cust == '(对所有已勾选)':
                               if st.session_state.get('chk_' + str(t), False):
                                    st.session_state['p_' + str(t)] = b_prc
                           elif t_cust == b_cust:
                          st.session_state['p_' + str(t)] = b_prc
              add_log('INFO', '批量设置运价: ' + str(b_prc) + '元，⽬标: ' + b_cust
              st.rerun()
...
def render_truck_row(t):
    ...
    this_df     = delivery_df[delivery_df['⻋号'] == t]
    curr_drv = st.session_state.get('custom_drv_' + str(t), mem_driver.get(st
    curr_prc = st.session_state.get('p_' + str(t), mem_price.get(str(this_df.
    prefix      = '       ' if (curr_drv and curr_prc > 0) else '      '
    c_chk, c_info, c_drv, c_prc = st.columns([2.5, 2, 2.5, 3])
    with c_chk:
        st.checkbox(prefix + str(t) + ' (' + str(count) + '趟)', key='chk_' +
    with c_info:
        with st.popover('            明细', use_container_width=True):
              for _, r in this_df.iterrows():
                      st.markdown(
                          '    **' + str(r['单号']) + '** |      ' + str(r['收货单位'])
                          '    ' + str(r['汇报专⽤名称']) + ' `' + str(r['净重']) + '吨
                      )
                      st.markdown('---')
    with c_drv:
    ...
    with c_prc:
        p_val = st.number_input(
              ...
              key='p_' + str(t), label_visibility='collapsed', placeholder='¥ 运
        price_map[t] = p_val
    st.markdown('<hr style="margin:0.25em 0;border-style:dashed;border-color:


for t in unique_trucks[:4]:
    render_truck_row(t)
if len(unique_trucks) > 4:
    with st.expander('↓ 展开剩余 ' + str(len(unique_trucks)-4) + ' 辆⻋'):
        for t in unique_trucks[4:]:
                      render_truck_row(t)
        ...
        for _, row in delivery_df.iterrows():
            ...
            new_freight_records.append({
                 '单号': row['单号'], '重⻋时间': row['重⻋时间'], '⻋号': t,
                 '收货单位': row['收货单位'], '货物名称': row['汇报专⽤名称'],
                 '净重': row['净重'], '司机姓名': d_name,
                 '运费单价': p_val, '运费⾦额': do_round(row['净重'] * p_val, 'freight'),
            })
        new_freight_df = pd.DataFrame(new_freight_records)
        freight_total     = new_freight_df['运费⾦额'].sum()
        add_log('INFO', '公司配送运费合计: ' + str(round(freight_total,2)) + '元')
    else:
        f_val = st.number_input('今⽇⽆配送，如有额外运费请填写(元):', value=0.0, step=1.0,
...
# 财务资⾦登记
# ============================================================
with st.expander('       财务资⾦登记 (收⼊ & 预存)', expanded=False):
    tab_income, tab_deposit = st.tabs(['       收⼊登记', '    预存登记'])
    ...
    def render_income_rows(prefix, type_options, rows_key, btn_key):
        ...
        for i in range(st.session_state[rows_key]):
            ...
            with c_c:
                 c_sel = st.selectbox('客户名称', cust_options, key=prefix + 'c_sel_' +
                 if c_sel == '     ⼿动输⼊新客户...':
                      c_name = st.text_input('新客户', key=prefix + 'c_new_' + str(i), l
                 else:
                      c_name = c_sel if c_sel != '(不录⼊)' else ''
            with c_t:
                 i_type = st.selectbox('类型', type_options, key=prefix + 't_' + str(i)
            with c_a:
                 i_amt = st.number_input('⾦额(元)', min_value=0.0, step=1.0, format='%
            with c_n:
                 i_note = st.text_input('备注', key=prefix + 'n_' + str(i), label_visib
            st.markdown('<hr style="margin:0.4em 0;border-style:dashed;border-color:#
            if c_name and i_amt > 0:
                     today_income_records.append({
                          '⽇期': report_date.strftime('%Y-%m-%d'),
                          '客户名称': c_name, '收⼊类型': i_type,
                          '⾦额': i_amt, '备注': i_note,
                     })
           col_add, _ = st.columns([1, 5])
           with col_add:
               if st.button('     新增⾏', key=btn_key):
                     st.session_state[rows_key] += 1
                     st.rerun()


    with tab_income:
           render_income_rows('inc_', ['微信','现⾦','银⾏卡','其他'], 'income_rows', 'add_
    with tab_deposit:
           render_income_rows('dep_', ['预存微信','预存银⾏卡','预存现⾦','预存备⽤⾦','预存其他


# ── 资⾦余额计算 ──────────────────────────────────
df_cash = df_sales_report[df_sales_report['过磅类型'].str.contains('现⾦', na=False)]
...
retail_wx_amt        = do_round(df_wx['⾦额'].sum(),     'retail')
retail_cash_amt = do_round(df_cash['⾦额'].sum(), 'retail')


if retail_wx_amt > 0:
    today_income_records.append({'⽇期': report_date.strftime('%Y-%m-%d'), '客户名称':
if retail_cash_amt > 0:
    today_income_records.append({'⽇期': report_date.strftime('%Y-%m-%d'), '客户名称':


for r in today_income_records:
    if ('预存' in r['收⼊类型'] or r['收⼊类型'] == '银⾏卡') and r['客户名称'] != '零售客户
           deposit_dict[r['客户名称']] = deposit_dict.get(r['客户名称'], 0.0) + float(r['⾦
...
bal_dict      = {}
for c in all_custs:
    spent = do_round(df_sign[df_sign['收货单位'] == c]['⾦额'].sum(), 'sign') if c in s
...
if not df_hist.empty and '重⻋时间' in df_hist.columns and '加⼯费' in df_hist.columns:
    df_hist['加⼯费'] = pd.to_numeric(df_hist['加⼯费'], errors='coerce').fillna(0)
    monthly_fee += do_round(
        df_hist[df_hist['重⻋时间'].astype(str).str.startswith(current_month, na=False)


add_log('INFO', '零售微信:' + str(retail_wx_amt) + '元 | 零售现⾦:' + str(retail_cash_am
...
# 汇报⽂本⽣成
# ============================================================
report = report_date.strftime('%y年%m⽉%d⽇') + ' ' + report_time + '\n'


if len(df_cash) == 0:
    report += '\n现⾦:⽆\n'
else:
    report += '\n现⾦:' + str(len(df_cash)) + '⻋' + fmt_weight(df_cash['净重'].sum())
    for prod, grp in df_cash.groupby('汇报专⽤名称'):
        report += prod + ':' + str(len(grp)) + '⻋' + fmt_weight(grp['净重'].sum()) +


if len(df_wx) == 0:
    report += '\n微信:⽆\n'
else:
    report += '\n微信:' + str(len(df_wx)) + '⻋' + fmt_weight(df_wx['净重'].sum()) + '吨
    for prod, grp in df_wx.groupby('汇报专⽤名称'):
        report += prod + ':' + str(len(grp)) + '⻋' + fmt_weight(grp['净重'].sum()) +
...
if len(df_sign) == 0:
    ...
    report += '\n签单:' + str(len(df_sign)) + '⻋' + fmt_weight(df_sign['净重'].sum())
    for cust, grp in sign_custs:
        report += cust + ':' + str(len(grp)) + '⻋' + fmt_weight(grp['净重'].sum()) +
        for prod, p_grp in grp.groupby('汇报专⽤名称'):
              report += prod + ':' + str(len(p_grp)) + '⻋' + fmt_weight(p_grp['净重'].s
        cust_money = do_round(grp['⾦额'].sum(), 'sign')
        report += '共⾦额:' + fmt_val(cust_money,'sign') + ' 元\n'
        report += '上⽇余额:' + fmt_val(orig_bal_dict.get(cust,0.0),'sign') + ' 元\n'
        if deposit_dict.get(cust, 0.0) > 0:
              report += '今⽇充值:' + fmt_val(deposit_dict[cust],'sign') + ' 元\n'
        report += '当⽇余额:' + fmt_val(bal_dict.get(cust,0.0),'sign') + ' 元\n\n'
...
    report += '【纯充值客户余额刷新】\n'
    for c in pure_depositors:
        report += c + ' 今⽇充值:' + fmt_val(deposit_dict[c],'sign') + '元 | 最新余额:'


total_money     = df_sales_report['⾦额'].sum()
unsold_count = len(df_daily_raw) - len(df_daily_raw[df_daily_raw['过磅类型'].astype(str
unsold_str      = ' (内含未销售单据/废料等 ' + str(unsold_count) + ' ⻋，已全量留底)' if unsol


report += ('1,当⽇销售共计:' + str(len(df_sales_report)) + ' ⻋' + fmt_weight(df_sales_r
              fmt_val(total_money,'sign') + ' 元,公司配送运费:' + fmt_val(freight_total,'fr
              '合计:' + fmt_val(total_money + freight_total,'sign') + ' 元。' + unsold_str
report += ('2,当⽇加⼯费:' + fmt_val(daily_fee,'fee') + ' 元,' +
              str(report_date.month) + '⽉1⽇-' + str(report_date.day) + '⽇加⼯费合计:' +


collection_parts       = []
custom_income_total = 0.0
if retail_wx_amt      > 0: collection_parts.append('微信零售:' + fmt_val(retail_wx_amt,'re
if retail_cash_amt > 0: collection_parts.append('现⾦零售:' + fmt_val(retail_cash_amt,'
...
# ============================================================
st.divider()
st.markdown('#####       第⼆步：复制每⽇汇报')
st.code(report, language='text', line_numbers=False)


# ── 组装新账本 ────────────────────────────────────
drop_cols = ['_pre_rounded', '_orig_idx', '汇报专⽤名称', '标准归类']
df_daily_raw.drop(columns=[c for c in drop_cols if c in df_daily_raw.columns], inplac


new_df_bal     = pd.DataFrame(list(bal_dict.items()), columns=['客户名称','余额'])
keep_cols      = ['单号','重⻋时间','⻋号','货物名称','净重','单价','⾦额','收货单位','过磅类型',
...
    today_income_df = pd.DataFrame(today_income_records) if today_income_records else pd.
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        df_rules.to_excel(writer,           sheet_name='加⼯费规则',       index=False)
        ...
        new_df_income.to_excel(writer,      sheet_name='财务收⼊明细',      index=False)


    success_key = 'success_' + report_date.strftime('%Y%m%d%H%M%S')
    if success_key not in st.session_state:
        add_log('INFO', '       账本⽣成成功，写⼊6张⼯作表，报表⽇期: ' + report_date.strftime('%Y
