    ...
    try:
        ...
        if '备注' in df_sales.columns:
            ...
            if delivery_mask.any():
                ...
                with col_b1:
                    if st.button("🔄 全选/反选", key="sel_all"):
                ...
                def render_truck_row(t):
                    ...
                    # 重新分配比例，恢复了加减号后，需要给运价框多留一点空间 (1: 4.2: 5.5: 5.5)
                    c_chk, c_info, c_drv, c_prc = st.columns([0.8, 4.2, 5.5, 5.5])
                    ...
                    with c_prc:
                        p_val = st.number_input(f"运价_{t}", value=curr_prc, format="%.2f", key=f"p_{t}", label_visibility="collapsed", placeholder="¥价")
                        price_map[t] = p_val
        ...
        with tab_income:
            ...
            for i in range(st.session_state.income_rows):
                # ★核心改动：一条记录分为两行，每行2列，给加减号留出极其宽敞的空间★
                c1, c2 = st.columns(2)
                c3, c4 = st.columns(2)
