    ...
    try:
        ...
        def read_sheet(name, cols):
            if name in sheets:
                ...
                return df
            add_log('WARN', '总账本缺少工作表: [' + name + ']，使用空表')
            return pd.DataFrame(columns=cols)
        ...
        if mixed_mask.any():
            ...
            for _, row in df_daily_raw.iterrows():
                ...
                if mat not in split_ratios:
                    # 🚀 致命 BUG 修复处：绝对不要给正常的单子发免死金牌，必须让它们去重算！
        ...
        if missing_mask.any():
            ...
            st.warning('⚠️ **第三关：' + str(missing_mask.sum()) + ' 条有效销售记录缺失单价，请补充**')
            cols = st.columns(4)
            price_inputs = {}
        ...
        if missing_rules:
            ...
            st.error('🛑 **第四关：发现 ' + str(len(missing_rules)) + ' 条未收录加工费规则，请补齐**')
            cols = st.columns(4)
            fee_inputs = {}
        ...
        all_known_custs = sorted(set(df_bal['客户名称'].dropna().astype(str).tolist() + hist_custs))
