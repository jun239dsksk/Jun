...
c1, c2 = st.columns(2)
with c1: db_file = st.file_uploader("📂 1. 上传【总账本.xlsx】", type=['xlsx'], help="包含客户余额、加工费规则、映射表等")
...
if db_file is not None and daily_file is not None:
    try:
        ...
        except: df_mapping = pd.DataFrame(columns=['原始名称', '标准名称'])
        ...
        # 🚨 检查未知物料 (既不在加工费规则里，也不在字典里，且不是(非销售/不计价)的)
        known_standards = set(df_rules['物料名称'].dropna().astype(str).str.strip().unique())
        known_standards.add("(非销售/不计价)") # 豁免白名单
        
        unknown_mats = [mat for mat in df_daily_raw['标准归类'].unique() if mat not in known_standards and str(mat).strip() != '']
        
        if unknown_mats:
            st.error("🛑 **发现未建立映射的未知物料（如异名规格、或废铁等非销售物资）！**", icon="🛑")
            st.markdown("请为它们指定一个标准的【父级归类】。如果它是废品或转场物资，请选择`(非销售/不计价)`，系统将不再强制要求单价。")
            ...
            # 下拉框选项：标准类目 + 豁免项
            map_options = ["(非销售/不计价)"] + sorted(list(set(df_rules['物料名称'].dropna().astype(str).str.strip().unique())))
            
            for i, u_mat in enumerate(unknown_mats):
                with cols[i % 3]:
                    mapping_inputs[u_mat] = st.selectbox(f"【{u_mat}】归类为：", ["(请选择...)"] + map_options, key=f"map_{u_mat}")
            
            if st.button("💾 保存归类映射，并下载更新后的总账本", type="primary"):
                for u_mat, sel_val in mapping_inputs.items():
                    if sel_val != "(请选择...)":
                        new_mapping_records.append({'原始名称': u_mat, '标准名称': sel_val})
                
                if new_mapping_records:
                    new_map_df = pd.DataFrame(new_mapping_records)
                    updated_df_mapping = safe_concat([df_mapping, new_map_df])
                    ...
                    with pd.ExcelWriter(output_temp, engine='openpyxl') as writer:
                        ...
                        updated_df_mapping.to_excel(writer, sheet_name='物料归类映射', index=False) # 保存字典
                    ...
                    st.download_button("📥 映射已保存！点击下载最新账本，然后重新上传", data=output_temp.getvalue(), file_name=f"更新字典_{report_date.strftime('%Y%m%d')}_DiBang总账本.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            st.stop() # 阻断运行
        ...
        new_amts = []
        for w, p, orig_a, t, pre_round, std_name in zip(df_daily_raw['净重'], df_daily_raw['单价'], orig_amts, types, df_daily_raw.get('_pre_rounded', [False]*len(df_daily_raw)), df_daily_raw['标准归类']):
            if pre_round:
                new_amts.append(orig_a)
            elif std_name == '(非销售/不计价)':
                new_amts.append(0.0) # 非销售物资金额强制为0
            else:
        ...
        # 提取报表 Base 数据（未归类，未被拆分的原始感观）
        df_report_base = df_daily_raw.copy()
        
        # 🚨 兜底：加工费自动学习与拦截（基于标准归类）
        missing_rules = []
        ...
        for _, r in unique_combinations.iterrows():
            ...
            if not is_exist: missing_rules.append({'物料名称': mat, '销售单价': price, '加工费单价': None})
        ...
        if missing_rules:
            ...
            st.error(f"🛑 **核算已中断！发现未记录的加工费规则：**\n\n`{added_mats}`\n\n请点击下方按钮下载更新后的总账本，填入【加工费单价】后，重新上传核算！", icon="🛑")
            ...
            with col_d1:
                st.download_button("📥 下载待补充加工费的总账本", data=output_temp.getvalue(), file_name=f"请补充加工费_{report_date.strftime('%Y%m%d')}_DiBang总账本.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", type="primary")
            st.stop()
        ...
        for prod, grp in df_cash.groupby('标准归类'):
        ...
        cols_to_keep = ['单号', '重车时间', '车号', '货物名称', '标准归类', '净重', '单价', '金额', '收货单位', '过磅类型', '备注', '加工费单价', '加工费', '备注2']
