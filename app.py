...
if db_file is not None and daily_file is not None:
    try:
        ...
        if mixed_mask.any():
            ...
            st.warning("⚠️ 检测到过磅单中包含混合料，已开启智能拆分模式：")
            cols = st.columns(2)
            split_ratios = {}
            for i, mat in enumerate(mixed_mats):
                parts = mat.split('+')
                p1 = parts[0].strip()
                with cols[i % 2]:
                    split_ratios[mat] = st.number_input(f"【{mat}】中 [{p1}] 占比(%)", min_value=1.0, max_value=99.0, value=50.0, step=10.0, key=f"split_{mat}")
            ...
            for idx, row in df_daily_raw.iterrows():
                ...
                if mat in split_ratios:
                    ...
                    w_orig = pd.to_numeric(row.get('净重', 0), errors='coerce') or 0.0
                    p_orig = pd.to_numeric(row.get('单价', 0), errors='coerce') or 0.0
                    a_orig = pd.to_numeric(row.get('金额', 0), errors='coerce') or 0.0
                    t = str(row.get('过磅类型', ''))
                    ...
                    r1['净重'] = w1
                    r1['金额'] = a1
                    r1['备注'] = f"原[{mat}]拆分:{mat1}{pct1:g}% | " + str(row.get('备注','')).replace('nan','')
                    r1['_pre_rounded'] = True # 标记已算好，跳过二次四舍五入
                    
                    r2['货物名称'] = mat2
                    r2['净重'] = w2
                    r2['金额'] = a2
                    r2['备注'] = f"原[{mat}]拆分:{mat2}{pct2:g}% | " + str(row.get('备注','')).replace('nan','')
        ...
        for idx, row in df_daily_raw.iterrows():
            if row.get('_pre_rounded', False):
                ...
                w = pd.to_numeric(row.get('净重', 0), errors='coerce') or 0.0
                p = pd.to_numeric(row.get('单价', 0), errors='coerce') or 0.0
                orig_a = pd.to_numeric(row.get('金额', 0), errors='coerce') or 0.0
                t = str(row.get('过磅类型', ''))
        ...
        expander_title = f"🔴 🚚 检测到 {len(df_sales[delivery_mask])} 车公司配送，点击展开分配" if has_delivery else "🚚 公司配送与额外运费分配"
        ...
        with st.expander("💰 财务资金登记 (点击展开录入收入与预存)", expanded=False):
            ...
            with tab_deposit:
                ...
                for i in range(st.session_state.deposit_rows):
                    ...
                    with c_t: 
                        i_type = st.selectbox(f"预存类型", ["预存微信", "预存银行卡", "预存现金", "预存备用金", "其他"], key=f"dep_t_{i}", label_visibility=label_vis)
                    with c_a: 
