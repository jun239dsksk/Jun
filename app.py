...
def show_logs_dialog():
    st.markdown("这里记录了本次系统运行的所有关键操作和异常拦截记录：")
    log_content = "\n".join(st.session_state.app_logs) if st.session_state.app_logs else "暂无日志记录"
    st.text_area("详细日志", value=log_content, height=300, label_visibility="collapsed")
    st.download_button(
        label="📥 导出日志 (.txt)", 
        data=log_content, 
        file_name=f"地磅系统日志_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt", 
...
# ⭐️ 左侧边栏新增日志查看按钮
if st.sidebar.button("📝 查看 / 导出系统日志", use_container_width=True):
    show_logs_dialog()
...
def add_driver_modal(t):
    ...
    if st.button("确认添加", type="primary", use_container_width=True):
        ...
        add_log("INFO", f"添加了新司机: {new_d.strip()}")
        st.rerun()

# ================= 5. 核心拦截流水线 =================
if db_file is not None and daily_file is not None:
    try:
        # 仅在初次读取时记日志，避免 Streamlit 循环刷新重复记录
        if f"loaded_{db_file.name}_{daily_file.name}" not in st.session_state:
            add_log("INFO", f"成功读取总账本: {db_file.name} | 明细单: {daily_file.name}")
            st.session_state[f"loaded_{db_file.name}_{daily_file.name}"] = True
        ...
        if mixed_mask.any():
            ...
            if not split_confirmed:
                add_log("WARN", "流程挂起：等待用户确认混合料拆分比例。")
                st.stop()
            else:
                if f"log_split_{report_date.strftime('%Y%m%d%H%M%S')}" not in st.session_state:
                    add_log("INFO", f"已按配置比例拆分混合料: {mixed_mats}")
                    st.session_state[f"log_split_{report_date.strftime('%Y%m%d%H%M%S')}"] = True
        ...
        if unknown_mats:
            ...
            if not map_confirmed:
                add_log("WARN", "流程挂起：发现未知物料，等待用户指定父级归类。")
                st.stop()
            ...
            for u_mat, sel_val in mapping_inputs.items():
                if sel_val != "(请选择...)":
                    ...
                    add_log("INFO", f"新建物料字典映射: [{u_mat}] -> [{std_name}]")
                ...
                with col_b2:
                    with st.popover("⚙️ 批量设置运价/司机", use_container_width=True):
                        ...
                        if st.button("应用运价", use_container_width=True):
                            ...
                            add_log("INFO", f"执行了批量设置运价操作: {b_prc}元。")
                            st.rerun()
        ...
        # 在流程终点记一条成功日志
        if f"success_{report_date.strftime('%Y%m%d%H%M%S')}" not in st.session_state:
            add_log("INFO", "✅ 所有核算完成，成功生成了新的总账本！")
            st.session_state[f"success_{report_date.strftime('%Y%m%d%H%M%S')}"] = True
