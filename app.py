...
st.session_state.app_logs.append(f”[{t}] [{level}] {msg}”)
...
filtered = logs if filter_level == "全部" else [l for l in logs if f"[{filter_level}]" in l]

if not filtered:
    st.info(f"暂无 {filter_level} 级别日志")
else:
    # 彩色高亮显示
    lines_html = []
    for line in filtered:
        ...
        lines_html.append(
            ...
            f'border-left:3px solid {color};padding-left:8px;margin-bottom:2px;'
            f'background:#f8f9fa;border-radius:0 4px 4px 0;">{line}</div>'
        )
...
st.download_button(
    "📥 导出日志 (.txt)",
    data=log_text,
    file_name=f"地磅日志_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
...
@media (max-width: 768px) {
    ...
    /* 两列行各占 ~50% */
    [data-testid="stHorizontalBlock"]:has([data-testid="column"]:nth-child(2)):not(:has([data-testid="column"]:nth-child(3))) [data-testid="column"] {
...
def do_round(val, category: str = “”) -> float:
if pd.isna(val):
return 0.0
val = float(val)
...
def fmt_val(v, category: str = “”) -> str:
if pd.isna(v):
return “0”
v = float(v)
flags = {“retail”: round_retail, “sign”: round_sign, “fee”: round_fee, “freight”: round_freight}
...
# 侧边栏模板下载（在侧边栏 with 块外调用会追加到侧边栏末尾，保持原逻辑）

with st.sidebar:
st.download_button(
label=“⬇ 下载空白模板”,
data=create_template(),
file_name=“地磅总账本_空白_V2.xlsx”,
use_container_width=True,
)
...
c1, c2 = st.columns(2)
with c1:
db_file    = st.file_uploader(“总账本 (.xlsx)”, type=[“xlsx”], label_visibility=“visible”)
with c2:
daily_file = st.file_uploader(“今日过磅单 (.xls/.xlsx/.csv)”, type=[“xls”,“xlsx”,“csv”], label_visibility=“visible”)
...
@st.dialog(“➕ 添加新司机”)
def add_driver_modal(truck_no: str):
new_d = st.text_input(f”【{truck_no}】的新司机姓名：”, placeholder=“输入姓名”)
if st.button(“确认添加”, type=“primary”, use_container_width=True):
if new_d.strip():
st.session_state[f”custom_drv_{truck_no}”] = new_d.strip()
add_log(“INFO”, f”新增司机: {new_d.strip()} (车号:{truck_no})”)
st.rerun()
...
try:
# ── 读取文件（去重日志）──────────────────────────
load_key = f”loaded_{db_file.name}_{daily_file.name}”
if load_key not in st.session_state:
add_log(“INFO”, f”读取总账本: {db_file.name}”)
add_log(“INFO”, f”读取今日过磅单: {daily_file.name}”)
...
```
    ...
    def read_sheet(name, cols):
        if name in sheets:
            ...
            if missing:
                add_log("WARN", f"工作表 [{name}] 缺少列: {missing}，已自动补全")
            return df if not missing else pd.DataFrame(columns=cols)
        add_log("WARN", f"总账本缺少工作表: [{name}]，使用空表")
    ...
    if "状态" in df_daily_raw.columns:
        ...
        if filtered_count:
            add_log("INFO", f"过滤作废/生产记录 {filtered_count} 条，剩余 {len(df_daily_raw)} 条")
    ...
    df_daily_raw["汇报专用名称"] = ""
    add_log("INFO", f"字段清洗完成，共 {len(df_daily_raw)} 条有效记录")
    ...
    if mixed_mask.any():
        ...
        add_log("WARN", f"检测到混合料 {len(mixed_mats)} 种: {list(mixed_mats)}，等待用户配置拆分比例")
        st.warning(f"⚠️ **第一关：检测到 {len(mixed_mats)} 种混合料，请配置拆分比例**")
        ...
        for mat in mixed_mats:
            ...
            with col_s1:
                pct1 = st.number_input(
                    f"【{mat}】[{p1}] 占比(%)",
                    min_value=1.0, max_value=99.0, value=50.0, step=1.0, key=f"s1_{mat}"
                )
            with col_s2:
                st.number_input(f"[{p2}] 占比(%)", value=100.0 - pct1, disabled=True, key=f"s2_{mat}")
        ...
        for _, row in df_daily_raw.iterrows():
            ...
            r1["备注2"] = r2["备注2"] = f"{mat1} {pct1:g}%+{mat2} {pct2:g}% 已拆分单号 {dh}"
        ...
        if not split_confirmed:
            ...
            split_log_key = f"log_split_{report_date.strftime('%Y%m%d%H%M%S')}"
            if split_log_key not in st.session_state:
                add_log("INFO", f"混合料拆分完成: {list(mixed_mats)}")
    ...
    if unknown_mats:
        add_log("WARN", f"发现未知物料 {len(unknown_mats)} 种: {unknown_mats}，等待用户归类")
        st.error(f"🛑 **第二关：发现 {len(unknown_mats)} 种未知物料，请指定父类**")
        ...
        for i, u_mat in enumerate(unknown_mats):
            with cols[i % 3]:
                mapping_inputs[u_mat] = st.selectbox(
                    f"【{u_mat}】", ["(请选择...)"] + map_options, key=f"map_{u_mat}"
                )
        ...
        for u_mat, sel_val in mapping_inputs.items():
            if sel_val != "(请选择...)":
                ...
                add_log("INFO", f"物料映射: [{u_mat}] → [{std_name}]")
        ...
        add_log("INFO", f"物料归类映射完成，新增 {len(new_mapping_records)} 条映射记录")

    # ── 修复收货单位 ─────────────────────────────────
    if "收货单位" not in df_daily_raw.columns:
        df_daily_raw["收货单位"] = ""

    def fix_shdw(row) -> str:
        shdw    = str(row.get("收货单位", "")).strip()
        gb_type = str(row.get("过磅类型", "")).strip()
        if shdw in ("", "nan"):
            if gb_type == "":                          return "内部单"
            if "微信" in gb_type or "现金" in gb_type: return "零售客户"
            return "未知客户"
        return shdw
    ...
    if missing_mask.any():
        ...
        add_log("WARN", f"检测到 {missing_mask.sum()} 条记录缺失单价，涉及 {len(missing_groups)} 个组合，等待用户补充")
        st.warning(f"⚠️ **第三关：{missing_mask.sum()} 条有效销售记录缺失单价，请补充**")
        cols = st.columns(4)
        price_inputs = {}
        for i, ((cust, mat), _) in enumerate(missing_groups):
            with cols[i % 4]:
                price_inputs[(cust, mat)] = st.number_input(
                    f"**{cust}**\n{mat} (元/吨)",
                    min_value=0.0, step=1.0, format="%.2f", key=f"miss_p_{cust}_{mat}"
                )
    ...
    for _, r in unique_combos.iterrows():
        ...
        if not exists:
            ...
            if row_val.empty or pd.isna(row_val.iloc[0]["加工费单价"]) or str(row_val.iloc[0]["加工费单价"]).strip() == "":
                missing_rules.append((mat, price))

    if missing_rules:
        add_log("WARN", f"发现 {len(missing_rules)} 条未收录加工费规则: {missing_rules}，等待用户补充")
        st.error(f"🛑 **第四关：发现 {len(missing_rules)} 条未收录加工费规则，请补齐**")
        cols = st.columns(4)
        fee_inputs = {}
        for i, (mat, price) in enumerate(missing_rules):
            with cols[i % 4]:
                fee_inputs[(mat, price)] = st.number_input(
                    f"**{mat}**\n单价:{price}元 → 加工费",
                    min_value=0.0, step=1.0, format="%.2f", key=f"miss_f_{mat}_{price}"
                )
        ...
        for (mat, price), fee in fee_inputs.items():
            add_log("INFO", f"新增加工费规则: [{mat}] 单价:{price} → 加工费:{fee}")
    ...
    add_log("INFO", f"核算完成：总销售额 {df_daily_raw['金额'].sum():.2f}，总加工费 {df_daily_raw['加工费'].sum():.2f}")
    ...
    all_known_custs = sorted(set(
        df_bal["客户名称"].dropna().astype(str).tolist()
        + df_hist["收货单位"].dropna().astype(str).tolist()
        if "收货单位" in df_hist.columns else df_bal["客户名称"].dropna().astype(str).tolist()
    ))
    ...
    expander_title = (
        f"🔴 🚚 检测到 {delivery_count} 车公司配送，点击展开分配"
        if has_delivery else "🚚 公司配送与额外运费"
    )

    with st.expander(expander_title, expanded=False):
        if has_delivery:
            add_log("INFO", f"检测到 {delivery_count} 车公司配送记录")
            ...
            with col_b1:
                if st.button("🔄 全选/反选", use_container_width=True):
                    ...
                    for t in unique_trucks:
                        st.session_state[f"chk_{t}"] = not curr
                    add_log("INFO", f"批量{'全选' if not curr else '反选'}配送车辆")
                    st.rerun()
            ...
            with col_b2:
                with st.popover("⚙️ 批量设置运价/司机", use_container_width=True):
                    ...
                    if st.button("应用司机(仅勾选)", use_container_width=True):
                        for t in unique_trucks:
                            if st.session_state.get(f"chk_{t}", False) and b_drv != "(未选择)":
                                st.session_state[f"custom_drv_{t}"] = b_drv
                        add_log("INFO", f"批量分配司机: {b_drv}")
                        st.rerun()
                    st.divider()
                    st.markdown("**按客户设置运价**")
                    ...
                    if st.button("应用运价", use_container_width=True):
                        for t in unique_trucks:
                            ...
                            if b_cust == "(对所有已勾选)":
                                ...
                                st.session_state[f"p_{t}"] = b_prc
                        add_log("INFO", f"批量设置运价: {b_prc}元，目标: {b_cust}")
                        st.rerun()
            ...
            def render_truck_row(t):
                ...
                with c_drv:
                    ...
                    d_sel = st.selectbox(
                        f"司机_{t}", opts, index=idx_sel,
                        key=f"d_sel_{t}", label_visibility="collapsed"
                    )
                    if d_sel == "➕ 手动输入新司机...":
                        add_driver_modal(t)
                    else:
                ...
                with c_prc:
                    p_val = st.number_input(
                        f"运价_{t}", value=curr_prc, step=1.0, format="%.2f",
                        key=f"p_{t}", label_visibility="collapsed", placeholder="¥ 运价"
                    )
                    price_map[t] = p_val

                st.markdown(
                    "<hr style='margin:0.25em 0;border-style:dashed;border-color:#eee;'/>",
                    unsafe_allow_html=True,
                )
            ...
            if len(unique_trucks) > 4:
                with st.expander(f"↓ 展开剩余 {len(unique_trucks)-4} 辆车"):
                    for t in unique_trucks[4:]:
                        render_truck_row(t)
            ...
            add_log("INFO", f"公司配送运费合计: {freight_total:.2f}元")
        else:
    ...
    with st.expander("💰 财务资金登记 (收入 & 预存)", expanded=False):
        ...
        def render_income_rows(prefix: str, tab_type: str, type_options: list, rows_key: str, btn_key: str):
            ...
            for i in range(st.session_state[rows_key]):
                ...
                with c_c:
                    c_sel = st.selectbox("客户名称", cust_options, key=f"{prefix}c_sel_{i}", label_visibility=lv)
                    if c_sel == "➕ 手动输入新客户...":
                        c_name = st.text_input("新客户", key=f"{prefix}c_new_{i}", label_visibility="collapsed", placeholder="客户名称")
                    else:
                        c_name = c_sel if c_sel != "(不录入)" else ""
                with c_t:
                    i_type = st.selectbox("类型", type_options, key=f"{prefix}t_{i}", label_visibility=lv)
                with c_a:
                    i_amt = st.number_input("金额(元)", min_value=0.0, step=1.0, format="%.2f", key=f"{prefix}a_{i}", label_visibility=lv)
                with c_n:
                    i_note = st.text_input("备注", key=f"{prefix}n_{i}", label_visibility=lv, placeholder="选填")
            ...
            with col_add:
                if st.button(f"➕ 新增行", key=btn_key):
                    st.session_state[rows_key] += 1
                    st.rerun()
        ...
        with tab_income:
            render_income_rows(
                "inc_", "income",
                ["微信","现金","银行卡","其他"],
                "income_rows", "add_income_btn"
            )
        with tab_deposit:
            render_income_rows(
                "dep_", "deposit",
                ["预存微信","预存银行卡","预存现金","预存备用金","预存其他"],
                "deposit_rows", "add_deposit_btn"
            )
    ...
    add_log("INFO", f"零售微信:{retail_wx_amt}元 | 零售现金:{retail_cash_amt}元 | 加工费:{daily_fee}元 | 运费:{freight_total}元")
    add_log("INFO", f"签单客户余额更新: {len(bal_dict)} 户")
    ...
    if len(df_sign) == 0:
        ...
        for cust, grp in sign_custs:
            ...
            if deposit_dict.get(cust, 0.0) > 0:
                report += f"今日充值:{fmt_val(deposit_dict[cust],'sign')} 元\n"
    ...
    if pure_depositors:
        ...
        for c in pure_depositors:
            report += f"{c} 今日充值:{fmt_val(deposit_dict[c],'sign')}元 | 最新余额:{fmt_val(bal_dict.get(c,0.0),'sign')}元\n\n"
    ...
    report += (
        f"1,当日销售共计:{len(df_sales_report)} 车{fmt_weight(df_sales_report['净重'].sum())} 吨 "
        f"{fmt_val(total_money,'sign')} 元,公司配送运费:{fmt_val(freight_total,'freight')}元 ,"
        f"合计:{fmt_val(total_money + freight_total,'sign')} 元。{unsold_str}\n"
    )
    report += (
        f"2,当日加工费:{fmt_val(daily_fee,'fee')} 元,"
        f"{report_date.month}月1日-{report_date.day}日加工费合计:{fmt_val(monthly_fee,'fee')} 元。\n"
    )
    ...
    for r in today_income_records:
        if r["客户名称"] != "零售客户":
            ...
            label  = f"{c_name}{r['收入类型']}" if c_name else r["收入类型"]
            collection_parts.append(f"{label}:{fmt_val(amt,'none')}元")
    ...
    success_key = f"success_{report_date.strftime('%Y%m%d%H%M%S')}"
    if success_key not in st.session_state:
        add_log("INFO", f"✅ 账本生成成功，写入6张工作表，报表日期: {report_date.strftime('%Y-%m-%d')}")
