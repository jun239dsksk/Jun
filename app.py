...
if db_file is not None and daily_file is not None:
    try:
        ...
        # 构建所有已知客户大名单 (从余额表、历史明细、以及字典表里收集)
        ...
        if unknown_custs:
            ...
            st.markdown('💡 如果它是老客户的别名（如分公司名、项目部），请选择对应老客户；如果是新客户，请选 `(新建客户)`。建档后可直接在最下方财务登记处看到它！')
        ...
        all_known_custs = sorted(set(df_bal['客户名称'].dropna().astype(str).tolist() + hist_custs + daily_custs))
