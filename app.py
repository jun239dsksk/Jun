...
if 'report_time_confirmed' not in st.session_state:
    st.session_state.report_time_confirmed = False
    st.session_state.report_date = datetime.now().date()
    st.session_state.report_time = '0:00-18:00'
...
@st.dialog('📅 设置新数据入账时间段')
def set_report_time_dialog():
    st.markdown('**检测到新导入的数据，请设置本次报表时间：**')
    d = st.date_input('1. 选择报表日期', value=st.session_state.report_date)
    t = st.radio('2. 选择时间段', ['0:00-18:00', '18:00-24:00'], horizontal=True)
    if st.button('✅ 确认并开始处理', type='primary', use_container_width=True):
        st.session_state.report_date = d
        st.session_state.report_time = t
        st.session_state.report_time_confirmed = True
        add_log('INFO', '设置报表入账时间: ' + d.strftime('%Y-%m-%d') + ' ' + t)
        st.rerun()
...
if db_file is not None and daily_file is not None:
    current_files_hash = db_file.name + '_' + str(db_file.size) + '_' + daily_file.name + '_' + str(daily_file.size)
    if st.session_state.last_files_hash != current_files_hash:
        st.session_state.report_time_confirmed = False
        st.session_state.last_files_hash = current_files_hash

    if not st.session_state.report_time_confirmed:
        set_report_time_dialog()
        st.stop()

    report_date = st.session_state.report_date
    report_time = st.session_state.report_time

    c_info1, c_info2 = st.columns([4, 1])
    with c_info1:
        st.info('📅 本次报表核算时间: **' + report_date.strftime('%Y-%m-%d') + '** | 时段: **' + report_time + '**')
    with c_info2:
        if st.button('✏️ 修改时间', use_container_width=True):
            st.session_state.report_time_confirmed = False
            st.rerun()
    ...
    try:
        ...
        # === 🌟 核心新功能：在线修改总账本 ===
        st.markdown('#### 🛠️ 在线修改总账本基础数据')
        with st.expander('点击展开：管理【客户余额】与【历史过磅明细】（修改后下方汇报自动重算）', expanded=False):
            tab_bal_edit, tab_hist_edit = st.tabs(['💰 客户余额管理', '📄 历史过磅明细管理'])
            with tab_bal_edit:
                st.caption('💡 提示：双击单元格直接修改。支持在最下方添加新行，选中行后按键盘Delete或通过右键菜单删除行。')
                df_bal = st.data_editor(df_bal, num_rows='dynamic', key='edit_bal_m1', use_container_width=True)
            with tab_hist_edit:
                st.caption('💡 提示：若发现以前的错单可在此直接修改或删除。系统会以此新数据为基准进行重算并存入新账本。')
                df_hist = st.data_editor(df_hist, num_rows='dynamic', key='edit_hist_m1', use_container_width=True)
...
# =====================================================
# 7. 历史/分段汇报提取模式 (仅上传总账本)
# =====================================================

elif db_file is not None and daily_file is None:
    st.info("📂 检测到仅上传了【总账本】，系统已进入【自选历史/分段汇报提取】模式")
    ...
    try:
        ...
        def read_sheet(name, cols):
            if name in sheets:
                df = pd.read_excel(xls, sheet_name=name)
                missing = [c for c in cols if c not in df.columns]
                if missing: return pd.DataFrame(columns=cols)
                return df
            return pd.DataFrame(columns=cols)

        df_bal     = read_sheet('客户余额',      ['客户名称', '余额'])
        df_hist    = read_sheet('过磅明细',      ['单号', '重车时间', '车号', '货物名称', '净重', '单价', '金额', '收货单位', '过磅类型', '备注', '加工费单价', '加工费', '备注2', '标准归类', '汇报专用名称'])
        df_freight = read_sheet('公司配送-运费', ['单号','重车时间','车号','收货单位','货物名称','净重','司机姓名','运费单价','运费金额'])
        df_income  = read_sheet('财务收入明细',  ['日期','客户名称','收入类型','金额','备注'])
        ...
        with st.expander('点击展开：管理【客户余额】与【历史过磅明细】（修改后下方汇报自动重算）', expanded=False):
            tab_bal_edit, tab_hist_edit = st.tabs(['💰 客户余额管理', '📄 历史过磅明细管理'])
            with tab_bal_edit:
                st.caption('💡 双击单元格修改。支持添加新行、右键删除行。')
                df_bal = st.data_editor(df_bal, num_rows='dynamic', key='edit_bal_m2', use_container_width=True)
            with tab_hist_edit:
                st.caption('💡 提示：修改此处的历史明细后，下方提取的汇报数据会瞬间自动同步重算！')
                df_hist = st.data_editor(df_hist, num_rows='dynamic', key='edit_hist_m2', use_container_width=True)

        st.markdown('#### 📅 选择提取参数')
        col_d, col_t = st.columns(2)
        with col_d:
            sel_date = st.date_input("1. 选择要提取的汇报日期", value=datetime.now().date())
        with col_t:
            sel_time = st.selectbox("2. 选择时间段", ["0:00-18:00", "18:00-24:00", "0:00-24:00"])

        st.markdown("---")
        st.info(f"🚀 正在实时提取 **{sel_date.strftime('%Y-%m-%d')}** ({sel_time}) 的汇报数据...")
        
        df_hist['重车时间_dt'] = pd.to_datetime(df_hist['重车时间'].apply(parse_excel_date), errors='coerce')
        df_freight['重车时间_dt'] = pd.to_datetime(df_freight['重车时间'].apply(parse_excel_date), errors='coerce')
        df_income['日期_dt'] = pd.to_datetime(df_income['日期'], errors='coerce')
        
        mask_date = df_hist['重车时间_dt'].dt.date == sel_date
        if sel_time == "0:00-18:00":
            mask_time = df_hist['重车时间_dt'].dt.hour < 18
        elif sel_time == "18:00-24:00":
            mask_time = df_hist['重车时间_dt'].dt.hour >= 18
        else:
            mask_time = True
        df_target = df_hist[mask_date & mask_time].copy()
        
        mask_f_date = df_freight['重车时间_dt'].dt.date == sel_date
        if sel_time == "0:00-18:00":
            mask_f_time = df_freight['重车时间_dt'].dt.hour < 18
        elif sel_time == "18:00-24:00":
            mask_f_time = df_freight['重车时间_dt'].dt.hour >= 18
        else:
            mask_f_time = True
        df_freight_target = df_freight[mask_f_date & mask_f_time].copy()
        freight_total = df_freight_target['运费金额'].sum()
        
        mask_i_date = df_income['日期_dt'].dt.date == sel_date
        df_income_target = df_income[mask_i_date].copy()
        
        df_target['过磅类型'] = df_target['过磅类型'].astype(str).replace('nan', '').str.strip()
        df_target['标准归类'] = df_target['标准归类'].astype(str).replace('nan', '').str.strip()
        df_target['汇报专用名称'] = df_target.apply(lambda r: r['货物名称'] if pd.isna(r.get('汇报专用名称')) or str(r.get('汇报专用名称')).strip() in ('', 'nan') else r['汇报专用名称'], axis=1)
        
        df_target['净重'] = pd.to_numeric(df_target['净重'], errors='coerce').fillna(0)
        df_target['金额'] = pd.to_numeric(df_target['金额'], errors='coerce').fillna(0)
        df_target['加工费'] = pd.to_numeric(df_target['加工费'], errors='coerce').fillna(0)
        
        df_sales_report = df_target[
            (df_target['过磅类型'] != '') & 
            (df_target['标准归类'] != '(非销售/不计价)')
        ].copy()
        ...
        for _, r in df_income_target.iterrows():
            if str(r.get('客户名称', '')).strip() == '零售客户' and '零售' in str(r.get('收入类型', '')):
                continue
            amt = float(r['金额']) if pd.notna(r['金额']) else 0.0
            c_name = str(r['客户名称']).replace('nan', '').strip()
            i_type = str(r['收入类型']).replace('nan', '').strip()
            
            if ('预存' in i_type or i_type == '银行卡') and c_name != '零售客户':
                deposit_dict[c_name] = deposit_dict.get(c_name, 0.0) + amt
                
            custom_income_total += amt
            label = c_name + i_type if c_name else i_type
            collection_parts.append(label + ':' + fmt_val(amt,'none') + '元')
        
        total_collection = retail_wx_amt + retail_cash_amt + custom_income_total
        
        current_bal_dict = dict(zip(df_bal['客户名称'], df_bal['余额']))
        sign_custs = df_sign.groupby('收货单位')
        
        daily_fee = df_sales_report['加工费'].sum()
        mask_month = (df_hist['重车时间_dt'].dt.month == sel_date.month) & (df_hist['重车时间_dt'].dt.year == sel_date.year) & (df_hist['重车时间_dt'].dt.date <= sel_date)
        monthly_fee = df_hist[mask_month]['加工费'].sum()
        
        report = sel_date.strftime('%y年%m月%d日') + ' ' + sel_time + '\n'
        ...
        if len(df_sign) == 0:
            ...
            for cust, grp in sign_custs:
                ...
                final_bal = current_bal_dict.get(cust, 0.0)
                depos = deposit_dict.get(cust, 0.0)
                start_bal = final_bal + cust_money - depos
                
                report += '共金额:' + fmt_val(cust_money,'sign') + ' 元\n'
                report += '上日余额:' + fmt_val(start_bal,'sign') + ' 元\n'
                if depos > 0:
                    report += '今日充值:' + fmt_val(depos,'sign') + ' 元\n'
                report += '当日余额(总账最新):' + fmt_val(final_bal,'sign') + ' 元\n\n'

        pure_depositors = [c for c in deposit_dict if c not in sign_custs.groups]
        if pure_depositors:
            report += '【纯充值客户余额刷新】\n'
            for c in pure_depositors:
                final_bal = current_bal_dict.get(c, 0.0)
                report += c + ' 今日充值:' + fmt_val(deposit_dict[c],'sign') + '元 | 最新余额:' + fmt_val(final_bal,'sign') + '元\n\n'

        total_money  = df_sales_report['金额'].sum()
        unsold_count = len(df_target) - len(df_sales_report)
        unsold_str   = ' (内含未销售单据/废料等 ' + str(unsold_count) + ' 车)' if unsold_count > 0 else ''

        report += ('1,当日销售共计:' + str(len(df_sales_report)) + ' 车' + fmt_weight(df_sales_report['净重'].sum()) + ' 吨 ' +
                   fmt_val(total_money,'sign') + ' 元,公司配送运费:' + fmt_val(freight_total,'freight') + '元 ,' +
                   '合计:' + fmt_val(total_money + freight_total,'sign') + ' 元。' + unsold_str + '\n')
        report += ('2,当日加工费:' + fmt_val(daily_fee,'fee') + ' 元,' +
                   str(sel_date.month) + '月1日-' + str(sel_date.day) + '日加工费合计:' + fmt_val(monthly_fee,'fee') + ' 元。\n')

        report += '3,当日合计收款:' + (','.join(collection_parts) if collection_parts else '0元') + ',共计:' + fmt_val(total_collection,'none') + ' 元\n'

        st.success("✅ 汇报已从总账本中成功提取！")
        st.code(report, language='text', line_numbers=False)
        
    except Exception as e:
        st.error("提取失败：" + str(e))
        add_log("ERROR", "自选汇报生成失败: " + str(e))
