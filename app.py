...
# 🌟 真正的响应式排版引擎：电脑端舒展一行，手机端自动 2x2 田字格
st.markdown("""
<style>
/* 1. 压缩行间距，提升屏幕利用率 */
.block-container {padding-top: 1rem !important; padding-bottom: 1rem !important; max-width: 100% !important;}

/* 2. 原生组件尺寸优化（保留加减号和小箭头） */
...
@media (max-width: 768px) {
    /* 阻断 Streamlit 默认的垂直堆叠，改为水平排列并允许换行 */
    [data-testid="stHorizontalBlock"] {
        flex-direction: row !important;
        flex-wrap: wrap !important;
    }
    /* 强制每个组件在手机上精确占据近 50% 宽度，形成完美的田字格 */
    [data-testid="column"] {
...
# 恢复默认的 2 列排列，CSS 会在手机端自动把它们变成 2x2 网格
c1, c2 = st.columns(2)
with c1: db_file = st.file_uploader("📂 1. 上传【总账本.xlsx】", type=['xlsx'], help="包含客户余额、加工费规则等基础数据的总账本")
    ...
    try:
        ...
        if '备注' in df_sales.columns:
            ...
            if delivery_mask.any():
                ...
                st.markdown(f"#### 🚚 今日有 {len(delivery_df)} 车公司配送")
                
                col_b1, col_b2, _ = st.columns([3, 4, 3])
                with col_b1:
                ...
                def render_truck_row(t):
                    ...
                    # 使用 4 列布局。
                    # 电脑端：一行完美展示
                    # 手机端（通过CSS）：自动切分为 上排 c_chk, c_info，下排 c_drv, c_prc
                    ...
                    with c_prc:
                        p_val = st.number_input(f"运价_{t}", value=curr_prc, format="%.2f", key=f"p_{t}", label_visibility="collapsed", placeholder="¥ 运价")
                        price_map[t] = p_val
        ...
        with tab_income:
            ...
            for i in range(st.session_state.income_rows):
                ...
                with c_a: 
                    i_amt = st.number_input(f"金额(元)", min_value=0.0, format="%.2f", key=f"inc_a_{i}", label_visibility=label_vis)
                with c_n:
        ...
        with tab_deposit:
            ...
            for i in range(st.session_state.deposit_rows):
                ...
                with c_t: 
                    i_type = st.selectbox(f"预存类型", ["预存微信", "银行卡", "预存现金", "其他"], key=f"dep_t_{i}", label_visibility=label_vis)
                with c_a: 
                    i_amt = st.number_input(f"金额(元) ", min_value=0.0, format="%.2f", key=f"dep_a_{i}", label_visibility=label_vis)
                with c_n:
