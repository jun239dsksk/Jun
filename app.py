...
def create_template():
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        pd.DataFrame(columns=['地磅打单名', '标准归类名']).to_excel(writer, sheet_name='物料归类映射', index=False)
        pd.DataFrame(columns=['单号', '重车时间', '车号', '货物名称', '标准归类名', '净重', '单价', '金额', '收货单位', '过磅类型', '备注', '加工费单价', '加工费', '备注2']).to_excel(writer, sheet_name='过磅明细', index=False)
...
st.sidebar.download_button(label="下载总账本模板", data=create_template(), file_name="地磅总账本_空白.xlsx")
...
c1, c2 = st.columns(2)
with c1: db_file = st.file_uploader("📂 1. 上传【总账本.xlsx】", type=['xlsx'], help="包含客户余额、加工费规则、归类字典等")
with c2: daily_file = st.file_uploader("📝 2. 上传【今日过磅单】", type=['xls', 'xlsx', 'csv'], help="当日的过磅明细单")
...
if db_file is not None and daily_file is not None:
    try:
        ...
        # 尝试读取字典表
        try: df_mapping = pd.read_excel(db_file, sheet_name='物料归类映射')
        except: df_mapping = pd.DataFrame(columns=['地磅打单名', '标准归类名'])
        ...
        # 拦截：补充缺失单价
        missing_mask = (df_daily_raw['单价'] == 0) & (df_daily_raw['金额'] == 0) & (df_daily_raw['过磅类型'].str.strip() != '')
        if missing_mask.any():
            st.warning("⚠️ 检测到以下销售记录缺失单价，请补充（系统将自动同步计算金额）：")
        ...
        # 混合料智能拆分引擎
        df_daily_raw['备注2'] = ''
        mixed_mask = df_daily_raw['货物名称'].astype(str).str.contains(r'\+', na=False)
        if mixed_mask.any():
            mixed_mats = df_daily_raw.loc[mixed_mask, '货物名称'].unique()
            st.warning("⚠️ 检测到过磅单中包含混合料，已开启明细拆分引擎：")
            split_ratios = {}
            for mat in mixed_mats:
                parts = mat.split('+')
                p1, p2 = parts[0].strip(), parts[1].strip() if len(parts)>1 else "其他料"
                
                st.markdown(f"**【{mat}】拆分比例配置：**")
                col_s1, col_s2 = st.columns([1, 1])
                with col_s1: pct1 = st.number_input(f"[{p1}] 占比(%)", min_value=1.0, max_value=99.0, value=50.0, step=1.0, key=f"split_1_{mat}")
                with col_s2: st.number_input(f"[{p2}] 占比(%)", value=100.0 - pct1, disabled=True, key=f"split_2_{mat}")
            ...
            for idx, row in df_daily_raw.iterrows():
                ...
                if mat in split_ratios:
                    ...
                    true_total_a = (w_orig * p_orig) if p_orig > 0 else (a_orig if a_orig>0 else 0)
                    exact_a1 = w1 * p_orig if p_orig > 0 else true_total_a * (pct1 / 100.0)
                    a1 = do_round(exact_a1, category)
                    a2 = round(a_orig - a1, 2)
                    
                    r1['货物名称'], r1['净重'], r1['金额'] = mat1, w1, a1
                    r1['备注2'] = f"{mat1} {pct1:g}%+{mat2} {pct2:g}% 已拆分单号 {dh}"
                    r1['_pre_rounded'] = True 
                    
                    r2['货物名称'], r2['净重'], r2['金额'] = mat2, w2, a2
                    r2['备注2'] = f"{mat1} {pct1:g}%+{mat2} {pct2:g}% 已拆分单号 {dh}"
                    r2['_pre_rounded'] = True
        ...
        # 核心算原金额
        orig_amts = df_daily_raw['金额']
        types = df_daily_raw['过磅类型']
        new_amts = []
        for idx, row in df_daily_raw.iterrows():
            if row.get('_pre_rounded', False):
        ...
        # 🌟 核心突破：物料归类智能拦截与自动学习引擎
        current_mappings = pd.concat([df_mapping, pd.DataFrame(st.session_state.get('new_mappings', []))], ignore_index=True)
        known_mats = current_mappings['地磅打单名'].dropna().unique() if not current_mappings.empty else []
        daily_mats = df_daily_raw['货物名称'].dropna().unique()
        
        unmapped = [m for m in daily_mats if m not in known_mats]
        
        if unmapped:
            st.error("🛑 **拦截到未归类的石料名称！请为其指定父集（大类）：**", icon="🛑")
            st.info("💡 若它是某个大类的分支（如 0-3渣 属于 高钙石料），请选择对应大类；若它是独立物料，请选择「(独立物料)」。归类后，加工费和报表将自动按大类汇总！")
            
            existing_parents = current_mappings['标准归类名'].dropna().unique().tolist() if not current_mappings.empty else []
            # 预设几个常用父集选项
            parent_opts = ["(独立物料)", "高钙石料", "白云石料"] + existing_parents
            parent_opts = list(dict.fromkeys(parent_opts)) # 去重
            parent_opts.append("➕ 新增自定义大类...")
            
            with st.form("mapping_form"):
                new_map_dict = {}
                cols = st.columns(3)
                for i, mat in enumerate(unmapped):
                    with cols[i % 3]:
                        st.markdown(f"**{mat}**")
                        sel = st.selectbox("归属于:", parent_opts, key=f"map_sel_{mat}", label_visibility="collapsed")
                        if sel == "➕ 新增自定义大类...":
                            custom_p = st.text_input("输入大类名称", key=f"map_custom_{mat}", placeholder="例如：花岗岩石料")
                            new_map_dict[mat] = custom_p
                        else:
                            new_map_dict[mat] = sel
                
                if st.form_submit_button("💾 保存归类并继续核算", type="primary", use_container_width=True):
                    valid = True
                    new_records = []
                    for m, p in new_map_dict.items():
                        if p == "➕ 新增自定义大类..." or str(p).strip() == "":
                            valid = False
                        else:
                            final_p = m if p == "(独立物料)" else p
                            new_records.append({'地磅打单名': m, '标准归类名': final_p})
                    
                    if valid:
                        if 'new_mappings' not in st.session_state: st.session_state.new_mappings = []
                        st.session_state.new_mappings.extend(new_records)
                        st.rerun()
                    else:
                        st.error("❌ 请完善新增的大类名称！")
            st.stop() # 拦截生效，等待用户归类
        ...
        # 映射生效：将所有子类自动转换为父类大名
        if not current_mappings.empty:
            map_dict = dict(zip(current_mappings['地磅打单名'], current_mappings['标准归类名']))
        else:
            map_dict = {}
        df_daily_raw['标准归类名'] = df_daily_raw['货物名称'].map(map_dict).fillna(df_daily_raw['货物名称'])

        df_report_base = df_daily_raw.copy()

        # 🚨 终极熔断：基于【父集大类】拦截缺失的加工费规则
        missing_rules = []
        valid_sales = df_daily_raw[df_daily_raw['过磅类型'].astype(str).str.strip() != '']
        
        # 加工费是根据“标准归类名”（父集）+ 单价 来查的
        unique_combinations = valid_sales[['标准归类名', '单价']].drop_duplicates()
        
        for _, r in unique_combinations.iterrows():
            mat = str(r['标准归类名']).strip()
            price = float(r['单价'])
            is_exist = False
            ...
            if not is_exist:
                missing_rules.append({'物料名称': mat, '销售单价': price, '加工费单价': None})
                
        if missing_rules:
            new_rules_df = pd.DataFrame(missing_rules)
            updated_df_rules = safe_concat([df_rules, new_rules_df])
            added_mats = ", ".join([f"[{r['物料名称']} - {r['销售单价']}元]" for r in missing_rules])
            
            st.error(f"🛑 **核算已中断！发现未记录的加工费规则：**\n\n`{added_mats}`\n\n为了保证财务数据绝对准确，系统已自动为您在【加工费规则】表中建档。**请点击下方按钮下载更新后的总账本，填入对应的【加工费单价】后，重新上传核算！**", icon="🛑")
            
            output_temp = io.BytesIO()
            with pd.ExcelWriter(output_temp, engine='openpyxl') as writer:
                df_bal.to_excel(writer, sheet_name='客户余额', index=False)
                updated_df_rules.to_excel(writer, sheet_name='加工费规则', index=False)
                current_mappings.to_excel(writer, sheet_name='物料归类映射', index=False) # 保存字典
                df_hist.to_excel(writer, sheet_name='过磅明细', index=False)
                df_freight.to_excel(writer, sheet_name='公司配送-运费', index=False)
                df_income.to_excel(writer, sheet_name='财务收入明细', index=False)
            ...
            with col_d1:
                st.download_button(
                    ...
                    file_name=f"请补充加工费_{report_date.strftime('%Y%m%d')}_DiBang总账本.xlsx",
        ...
        def calc_fee_price(row):
            gb_type = str(row.get('过磅类型', '')).strip()
            if gb_type == '': return 0.0 
            mat = str(row['标准归类名']).strip()
            price = float(row['单价'])
            match = df_rules[(df_rules['物料名称'].astype(str).str.strip() == mat) & (df_rules['销售单价'].astype(float) == price)]
            return float(match.iloc[0]['加工费单价'])
        ...
        # 提取报表数据
        df_sales_report = df_report_base[df_report_base['过磅类型'].astype(str).str.strip() != ''].copy()
        ...
        daily_fee = df_daily_raw[df_daily_raw['过磅类型'].astype(str).str.strip() != '']['加工费'].sum()
        ...
        # ⭐️ 核心：按照 标准归类名（父集） 进行汇总汇报
        for prod, grp in df_cash.groupby('标准归类名'):
        ...
        # ---------------- 组装新账本 ----------------
        if '_pre_rounded' in df_daily_raw.columns:
            df_daily_raw = df_daily_raw.drop(columns=['_pre_rounded'])
            
        new_df_bal = pd.DataFrame(list(bal_dict.items()), columns=['客户名称', '余额'])
        cols_to_keep = ['单号', '重车时间', '车号', '货物名称', '标准归类名', '净重', '单价', '金额', '收货单位', '过磅类型', '备注', '加工费单价', '加工费', '备注2']
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            current_mappings.to_excel(writer, sheet_name='物料归类映射', index=False) # 写入并保存字典！
