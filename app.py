    ...
    try:
        ...
        if '备注' in df_sales.columns:
            ...
            if delivery_mask.any():
                ...
                with col_batch1: st.button("全选车辆", key="select_all_truck", size="small")
                with col_batch2: st.button("批量设置运价", key="batch_price", size="small")
        ...
        with tab_income:
            ...
            with col_add:
                if st.button("➕ 新增收入行", key="add_income_btn", size="small"):
        ...
        with tab_deposit:
            ...
            col_add, _ = st.columns([1,5])
            with col_add:
                if st.button("➕ 新增预存行", key="add_deposit_btn", size="small"):
        ...
        st.button("📋 汇报已生成，请在上方框内右上角点击复制按钮", key="copy_report", size="small", disabled=True)
