...
if db_file is not None and daily_file is not None:
    try:
        ...
        if '加工费规则' in xls.sheet_names:
            ...
            df_rules = pd.DataFrame(columns=['物料名称', '销售单价', '加工费单价'])
        ...
        if '物料归类映射' in xls.sheet_names:
            ...
            df_mapping = pd.DataFrame(columns=['原始名称', '标准名称'])
            ...
            if st.button("💾 保存归类映射，并下载更新后的总账本", type="primary"):
                ...
                if new_mapping_records:
                    ...
                    with pd.ExcelWriter(output_temp, engine='openpyxl') as writer:
                        ...
                        updated_df_mapping.to_excel(writer, sheet_name='物料归类映射', index=False)
