    ...
    try:
        ...
        if mixed_mask.any():
            ...
            for mat in mixed_mats:
                ...
                with col_s1:
                    pct1 = st.number_input(f"[{p1}] 占比(%)", min_value=1.0, max_value=99.0, value=50.0, step=1.0, key=f"split_1_{mat}")
                with col_s2:
                    # 自动计算，强制锁定
                    st.number_input(f"[{p2}] 占比(%)", value=100.0 - pct1, disabled=True, key=f"split_2_{mat}")
        ...
        def calc_fee_price(row):
            ...
            if not match.empty: return float(match.iloc[0]['加工费单价'])
            return 0.0
